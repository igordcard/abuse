...
- The string reaches the C++ code via `src/abuse_version.h.in` →
  `${CMAKE_BINARY_DIR}/abuse_version.h` (generated by `configure_file`),
  **not** via `add_definitions(-DPACKAGE_VERSION=...)`. The header-based
  route is required because the default Makefile generator does not track
  command-line changes as compile-unit dependencies, so a `-D` define that
  changes between configures does not force recompilation and a stale
  version silently ends up in the binary. `configure_file` only rewrites
  the header when its content changes, so the bake-in is both fresh and
  cache-friendly: incremental builds only recompile version-using
  translation units when the version actually moved.
